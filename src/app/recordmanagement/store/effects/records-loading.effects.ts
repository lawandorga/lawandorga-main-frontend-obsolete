...
export class RecordsLoadingEffects {
                    ...
                    mergeMap(response => {
                        ...
                        Object.values(response).map(record => {
                            ...
                            if (record['has_permission']) {
                                ...
                                loadedRecords.push(
                                    RestrictedRecord.getRestrictedRecordFromJson(record)
                                );
                            }
                        });
            ...
            if (this.appSB.isAuthenticated()) {
                return from(
                    this.http.get(RECORDS_STATICS_API_URL).pipe(
                        catchError(error => {
                            this.snackbarService.showErrorSnackBar(
                                `error at loading record statics: ${error.error.detail}`
                            );
                            return [];
                        }),
                        mergeMap(
                                ...
                                return [
                                    ...
                                    },
                                    {
                                        ...
                                        payload: State.getStatesFromJsonArray(
                                            response.record_states
                                        )
                                    },
                                    {
                                        type: SET_COUNTRY_STATES,
                                        payload: State.getStatesFromJsonArray(
                                            response.country_states
                                        )
                                    }
                                ];
                            }
            ...
            return from(
                this.http
                    ...
                    .pipe(
                        catchError(error => {
                            this.snackbarService.showErrorSnackBar(
                                `error at loading client possibilities: ${error.error.detail}`
                            );
                            return [];
                        }),
                        mergeMap(response => {
    ...
    startLoadingSpecialRecord = this.actions.pipe(
        ...
        switchMap((id: string) => {
            return from(
                this.http
                    .get(GetSpecialRecordApiURL(id), AppSandboxService.getPrivateKeyPlaceholder())
                    .pipe(
                        catchError(error => {
                            this.snackbarService.showErrorSnackBar(
                                `error at loading special record: ${error.error.detail}`
                            );
                            return [];
                        }),
        ...
        switchMap(() => {
            return from(
                this.http.get(RECORD_PERMISSIONS_LIST_API_URL).pipe(
                    catchError(error => {
                        this.snackbarService.showErrorSnackBar(
                            `error at loading record permission list: ${error.error.detail}`
                        );
                        return [];
                    }),
                    mergeMap(response => {
        ...
        switchMap(() => {
            return from(
                this.http.get(RECORD_DELETIONS_API_URL).pipe(
                    catchError(error => {
                        this.snackbarService.showErrorSnackBar(
                            `error at loading record deletion list: ${error.error.detail}`
                        );
                        return [];
                    }),
                    mergeMap(response => {
        ...
        switchMap(() => {
            return from(
                this.http.get(RECORD_POOL_API_URL).pipe(
                    catchError(error => {
                        this.snackbarService.showErrorSnackBar(
                            `error at loading record pool: ${error.error.detail}`
                        );
                        return [];
                    }),
                    mergeMap(response => {
