...
export class NotificationTextBuilderHelper {
  static generateNotificationText(source_user_name: string, event_type: NotificationType, text: string): string {
    ...
    if (event_type === NotificationType.RECORD__UPDATED) {
      return `${source_user_name} updated ${text.split(',').length > 1 ? 'fields' : 'field'} ${
